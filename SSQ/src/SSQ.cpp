...
// NOTE (Eyaz Rehman): Unicode Patch, as it is not supported
#ifdef _UNICODE
#undef _UNICODE
#endif
#ifdef UNICODE
...
// NOTE (Eyaz Rehman): Get rid of unsupported linker directives
#ifndef __cplusplus
...
void WINAPI SSQ_OutputDebugString(BYTE index, BYTE winsock, BYTE external, DWORD address);
...
char* WINAPI SSQ_FormatBatchReply(PSSQ_BATCH_REPLY batch_reply, int index)
{
	...
	if (HIWORD(batch_reply) == 0 || batch_reply->data_size == 0 || batch_reply->data == 0 || index>(batch_reply->num_servers - 1) || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_GetBatchReply(BYTE region, char* filter)
{
	...
	if ((region>7 && region<255) || HIWORD(filter) == 0 || HIWORD(Callback) == 0 || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_GetInfoReply(PSSQ_INFO_REPLY info_reply)
{
	...
	if (HIWORD(info_reply) == 0 || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_GetPlayerReply(PSSQ_PLAYER_REPLY player_reply)
{
	...
	if (HIWORD(player_reply) == 0 || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_GetRconReply(char* password, char* command)
{
	...
	if (HIWORD(password) == 0 || HIWORD(command) == 0 || HIWORD(Callback) == 0 || is_initialized == FALSE)
	{
...
char* WINAPI SSQ_GetRuleName(PSSQ_RULES_REPLY rules_reply, short index)
{
	...
	if (HIWORD(rules_reply) == 0 || rules_reply->data_size == 0 || HIWORD(rules_reply->data) == 0 || index>(rules_reply->num_rules - 1) || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_GetRulesReply()
{
	...
	if (HIWORD(Callback) == 0 || is_initialized == FALSE)
	{
...
{
	if (exit == FALSE&&is_initialized == FALSE)
	{
	...
	else if (exit == TRUE&&is_initialized == TRUE)
	{
...
BOOL WINAPI SSQ_SetGameServer(char* address)
{
	...
	if (HIWORD(address) == 0 || SSQ_GetIpPort(address, &socket_address) == FALSE || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_SetLogStatus(BOOL status, USHORT port)
{
	...
	if (status == log_status || HIWORD(Callback) == 0 || is_initialized == FALSE)
	{
...
BOOL WINAPI SSQ_Startup()
{
	...
	handle_module = GetModuleHandle(NULL);
